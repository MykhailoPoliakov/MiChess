...
class Bot:
    ...
    def _evaluate_game(self, game: Game, start_pos: str, end_pos: str) -> float:
        weight: float = 5
        ...
        if not game.check:
            ...
            if game.moves['comb_op_cover'][start_pos] == 'x ':
                ...
                elif game.moves['comb_op_cover'][end_pos] == 'x ' and game.moves['comb_cover'][end_pos] == 'x ':
                    weight += 30
            ...
            if game.moves["comb_cover"][start_pos] != 'x ' and game.moves["comb_cover"][end_pos] == 'x ':
                weight += 150

            # if you can castle
            if game.board[start_pos][1] == 'k' and abs(abs(int(start_pos[0])) - abs(int(end_pos[0]))) == 2:
                weight += 200_000

            # if piece is moving forward
            if ((int(start_pos) - int(end_pos)) < 0 and game.player == 'w') or (
                    (int(start_pos) - int(end_pos)) > 0 and game.player == 'b'):
                weight += 100

                # if piece is moving forward and to the center
                if int(end_pos[0]) in [ 3, 4, 5, 6 ]:
                    weight += 200

                # move pawn to promotion
                if game.board[ start_pos ][1] == 'p' and ((game.player == 'w' and start_pos in ['6', '7']) or
                                                           (game.player == 'b' and start_pos in ['2', '3'])):
                    weight += 5000
